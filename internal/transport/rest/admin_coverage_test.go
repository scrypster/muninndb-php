...
func TestHandleEmbedStatus_EmptyProvider(t *testing.T) {
	store := newTestAuthStore(t)
	srv := NewServer("localhost:0", &MockEngine{}, store, nil, nil, EmbedInfo{}, nil, "")
...
func TestHandleGetPluginConfig_WithDataDir(t *testing.T) {
	...
	store := newTestAuthStore(t)
	srv := NewServer("localhost:0", &MockEngine{}, store, nil, nil, EmbedInfo{}, nil, dir)
...
func TestHandleHello_InvalidJSON(t *testing.T) {
	srv := NewServer("localhost:0", &MockEngine{}, nil, nil, nil, EmbedInfo{}, nil, "")
